...
def metrics_summary(db: Session = Depends(get_db)):
    ...
    return MetricsSummary(total_jobs=total, successful_jobs=ok, failed_jobs=failed, success_rate=ok/total if total else 0.0, repair_success_rate=repair_ok/repair_cases if repair_cases else 0.0, average_attempts=sum(m.attempts for m in metrics)/total if total else 0.0, average_latency_seconds=sum(m.latency_seconds for m in metrics)/total if total else 0.0, total_llm_calls=sum(m.llm_calls or 0 for m in metrics), total_tokens=sum(m.total_tokens or 0 for m in metrics), total_estimated_cost_usd=sum(m.estimated_cost_usd or 0.0 for m in metrics))
